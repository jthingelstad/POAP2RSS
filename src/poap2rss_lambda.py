...
INACTIVITY_THRESHOLD_WEEKS = int(
    os.environ.get("INACTIVITY_THRESHOLD_WEEKS", "12")
...
class POAPAPIClient:
    ...
    def __init__(self):
        ...
        self.session.headers.update({"User-Agent": "POAP2RSS/1.0", "Accept": "application/json"})
    ...
    def _get_access_token(self) -> str:
        ...
        try:
            response = self.session.post(
                auth_url, json=auth_data, headers={"Content-Type": "application/json"}
            )
    ...
    def _make_request(self, endpoint: str, params: Dict = None) -> Dict:
        ...
        headers = {"Authorization": f"Bearer {token}", "X-API-Key": POAP_API_KEY}
...
class RSSFeedGenerator:
    ...
    def _add_channel_metadata(self, channel: Element, event_details: Dict, feed_type: str):
        ...
        SubElement(channel, "lastBuildDate").text = formatdate(
            timeval=time.time(), localtime=False, usegmt=True
        )
    ...
    def _add_event_description_item(self, channel: Element, event_details: Dict):
        ...
        if event_details.get("image_url"):
            description_html += (
                f'<img src="{event_details["image_url"]}" width="500" height="500" />'
            )
        ...
        SubElement(
            item, "guid"
        ).text = f"https://poap.gallery/drops/{event_details.get('id', 'unknown')}"
        SubElement(
            item, "link"
        ).text = f"https://poap.gallery/drops/{event_details.get('id', 'unknown')}"
            ...
            if event_details.get(date_field):
                try:
                    ...
                    if date_str.endswith("Z"):
                        ...
                        event_datetime = datetime.fromisoformat(date_str).replace(
                            tzinfo=timezone.utc
                        )
        ...
        SubElement(item, "pubDate").text = formatdate(
            timeval=event_timestamp, localtime=False, usegmt=True
        )
        ...
        if poap.get("created"):
            try:
                ...
                if created_str.endswith("Z"):
                    ...
                    created_datetime = datetime.fromisoformat(created_str).replace(
                        tzinfo=timezone.utc
                    )

                SubElement(item, "pubDate").text = formatdate(
                    timeval=created_datetime.timestamp(), localtime=False, usegmt=True
                )
...
"""
        ...
        SubElement(
            item, "guid"
        ).text = f"https://collectors.poap.xyz/token/{poap.get('tokenId', '')}"
        SubElement(
            item, "link"
        ).text = f"https://collectors.poap.xyz/token/{poap.get('tokenId', '')}"
    ...
    def _check_and_add_inactivity_alert(
        self, channel: Element, event_details: Dict, poaps: List[Dict]
    ):
        """Check for inactivity and add alert if needed"""
        if not poaps:
            ...
            if poap.get("created"):
                try:
                    ...
                    if created_str.endswith("Z"):
                        ...
                        # Assume UTC if no timezone info
                        claim_date = datetime.fromisoformat(created_str).replace(
                            tzinfo=timezone.utc
                        )
        ...
        if weeks_since_last_claim >= INACTIVITY_THRESHOLD_WEEKS:
            ...
            SubElement(
                item, "description"
            ).text = f"""<p>There have been no new POAP claims for this event in {weeks_since_last_claim} weeks.</p>
...
"""
            ...
            SubElement(
                item, "guid"
            ).text = f"https://www.poap2rss.com/inactive.html?event={event_details.get('id', 'unknown')}&week={weeks_since_last_claim}"
            SubElement(
                item, "link"
            ).text = f"https://www.poap2rss.com/inactive.html?event={event_details.get('id', 'unknown')}&week={weeks_since_last_claim}"
            SubElement(item, "pubDate").text = formatdate(
            ...
            logger.info(
                f"No inactivity alert needed (only {weeks_since_last_claim} weeks since last claim)"
            )
    ...
    try:
        ...
        if len(path_parts) < 2:
            return {
                ...
                "body": json.dumps(
                    {"error": "Invalid path. Use /event/{id} or /address/{address}"}
                ),
            }
        ...
        if feed_type == "event":
            try:
                ...
                rss_xml = feed_generator.generate_event_feed(
                    event_id, suppress_inactivity_alert=suppress_warning
                )
            except ValueError:
                return {
                    "statusCode": 400,
...
if __name__ == "__main__":
    # Test event
    test_event = {"path": "/event/12345", "httpMethod": "GET"}
